...
export const acm_info: ToolDefinition = tool({
  description: `Show ACM plugin status: version, session info, context metrics, structural breakdown by source, compaction state, and runtime-telemetry status.

The structural breakdown shows where context tokens go across sources: system prompt,
messages, OM slab, ACM injections, AGENTS.md, MCP schemas. This is the full
"context anatomy" view — use it to identify which levers move the needle.`,
  ...
  async execute(_params, ctx) {
    ...
    if (cached?.systemSegments) {
      ...
      if (skillsMatch) skillsChars = skillsMatch[0].length
    }
    const systemOnlyChars = Math.max(0, systemChars - skillsChars)
    out += `System prompt:    ${String(estTokens(systemOnlyChars)).padStart(8)} tok  [measured]\n`
    if (skillsChars > 0) {
      out += `Skills block:     ${String(estTokens(skillsChars)).padStart(8)} tok  [measured]\n`
    }

    // 2. Messages [measured]
    // Use character-based estimation per message — t.total from OpenCode metadata
    // is cumulative context size (not per-message), so summing it would wildly
    // overcount. The real total is shown in the Context section above.
    const ACTIVE_WINDOW = 200
    const recentMsgs = activeMsgs.slice(-ACTIVE_WINDOW)
    let activeChars = 0
    for (const msg of recentMsgs) {
      activeChars += messageChars(msg).chars
    }
    const activeTokensEst = estTokens(activeChars)
    const windowNote = activeMsgs.length > ACTIVE_WINDOW
      ? ` (last ${ACTIVE_WINDOW} of ${activeMsgs.length})`
      : ""
    out += `Active messages:  ${String(activeTokensEst).padStart(8)} tok  [estimated]${windowNote}\n`
    ...
    if (omChars > 0) {
      out += `OM slab:          ${String(estTokens(omChars)).padStart(8)} tok  [measured]\n`
    }
    ...
    if (telemetryChars > 0) {
      out += `ACM telemetry:    ${String(estTokens(telemetryChars)).padStart(8)} tok  [measured]\n`
    }

    // 5. AGENTS.md [estimated]
    let agentsMdChars = 0
    const agentsMdPaths = [
    ...
    if (agentsMdChars > 0) {
      out += `AGENTS.md:        ${String(estTokens(agentsMdChars)).padStart(8)} tok  [estimated]\n`
    }

    // 6. MCP schemas [estimated]
    let mcpToolChars = 0
    ...
    for (const p of configPaths) {
      try {
        ...
        for (const [, serverConfig] of Object.entries(config.mcp ?? {})) {
          if ((serverConfig as any).enabled === false) continue
          mcpServerCount++
          mcpToolChars += 10 * 400
        }
      } catch {}
    ...
    if (mcpToolChars > 0) {
      out += `MCP schemas:      ${String(estTokens(mcpToolChars)).padStart(8)} tok  [estimated]\n`
      out += `  (~${mcpServerCount} servers)\n`
    }

    // 7. Unaccounted
    const knownChars = systemOnlyChars + skillsChars + activeChars + omChars + telemetryChars + agentsMdChars + mcpToolChars
    if (total > 0) {
      const unaccounted = total - estTokens(knownChars)
      if (unaccounted > 0) {
        out += `Unaccounted:      ${String(unaccounted).padStart(8)} tok\n`
        out += `  (tool defs, formatting, etc.)\n`
      }
    }
